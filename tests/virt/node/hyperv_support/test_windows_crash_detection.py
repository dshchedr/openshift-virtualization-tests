...
@pytest.fixture()
def trigger_windows_crash(windows_vm_with_panic_device):
...
def wait_for_guest_panicked_event(vm, admin_client):
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            marks=pytest.mark.polarion("CNV-00000"),  # TODO: Replace with actual Polarion ID from Jira task
        ),
    ],
    indirect=True,
)
...
def test_windows_crash_detection_with_hyperv_panic(
    ...
    admin_client,
    windows_vm_with_panic_device,
    trigger_windows_crash,
):
    wait_for_guest_panicked_event(vm=windows_vm_with_panic_device, admin_client=admin_client)
